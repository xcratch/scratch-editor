...
class VersionRow extends React.Component {
    ...
    handleClickPlay () {
        this.props.onPlay(this.props.timestamp);
    }
    handleClickSeeInside () {
        this.props.onSeeInside(this.props.timestamp);
    }
        ...
        return (
            <div className={styles.versionRow}>
                {this.props.graphInfo && (
                    <div
                        className={styles.gitGraphContainer}
                        style={{width: svgWidth}}
                    >
                        <svg
                            width={svgWidth}
                            height="100%"
                            className={styles.gitGraphSvg}
                        >
                            ...
                            {this.props.graphInfo.incomingLines.map(line => (
                                <path
                                    key={`inc-${line.fromCol}`}
                                    d={`M ${line.fromCol * 20 + 10} 0 C ${line.fromCol * 20 + 10} 20, ${this.props.graphInfo.nodeColumn * 20 + 10} 20, ${this.props.graphInfo.nodeColumn * 20 + 10} 30`}
                                    stroke={line.color}
                                    strokeWidth="2"
                                    fill="none"
                                />
                            ...
                            {this.props.graphInfo.outgoingLines.map(line => (
                                <line
                                    key={`out-${line.fromCol}`}
                                    x1={line.fromCol * 20 + 10}
                                    y1="30"
                                    x2={line.toCol * 20 + 10}
                                    y2="100%"
                                    stroke={line.color}
                            ...
                            <circle
                                cx={this.props.graphInfo.nodeColumn * 20 + 10}
                                cy="30"
                                r="5"
                                fill={this.props.graphInfo.nodeColor}
                            />
                        </svg>
                    </div>
                )}
                                ...
                                {this.props.diff.code && (
                                    <span className={`${styles.diffBadge} ${styles.diffBadgeCode}`}>
                                        <FormattedMessage
                                            ...
                                            id="xcratch.projectHistory.diffCode"
                                        />
                                    </span>
                                )}
                                {this.props.diff.assets && (
                                    <span className={`${styles.diffBadge} ${styles.diffBadgeAssets}`}>
                                        <FormattedMessage
                                            ...
                                            description="Assets changed"
                                            id="xcratch.projectHistory.diffAssets"
                                        />
                                    </span>
                                )}
                                {this.props.diff.sprites !== 0 && (
                                    <span className={`${styles.diffBadge} ${styles.diffBadgeSprites}`}>
                                        ...
                                        {' '}<FormattedMessage
                                            ...
                                            description="Sprites changed"
                                            id="xcratch.projectHistory.diffSprites"
                                        />
                                    </span>
                                )}
                            </div>
                ...
                <textarea
                    ...
                    placeholder={this.props.canManage ? this.props.commentPlaceholder : ''}
                    readOnly={!this.props.canManage}
                    rows={1}
                ...
                <div className={styles.versionActions}>
                    ...
                    {this.props.canPlay && (
                        ...
                        >
                            <FormattedMessage
                                defaultMessage="Play this version"
                                description="Button to open a past version in the player, read-only"
                                id="xcratch.projectHistory.playVersion"
                            />
                        </button>
                    )}
                    {this.props.canSeeInside && (
                        <button
                            className={styles.itemButton}
                        ...
                        >
                            <FormattedMessage
                                ...
                                description="Button to open a past version in the editor view, read-only"
                                id="xcratch.projectHistory.seeInsideVersion"
                            />
                        </button>
                    )}
                    {this.props.canManage && (
